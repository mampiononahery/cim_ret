...
<body onselectstart="return false" oncontextmenu="return false" ondragstart="return false" onMouseOver="window.status=''; return true;">
  <table width="991px" border="0" align="center" cellpadding="2" cellspacing="1">
    ...
    <tr bgcolor="#000000" class="table_entete"> 
      ...
      <td>Pseudos</td>
      <td>E-mails</td>
      <td colspan="3">T&eacute;l&eacute;phones</td>
      <td align="center">Tarifs unitaires</td>
  ...
  ?>
    <tr <?php if ($i%2==0) echo 'bgcolor="#B0DBFD"'; else echo 'bgcolor="#D8EDFE"';?> class="table_contenu"> 
      ...
      <td><div align="center"><a href="javascript:;" onClick="supprimer('<?php echo $id_client; ?>','<?php echo $prenom_client.' '.$nom_client; ?>');" title="supprimer <?php echo $prenom_client; ?> <?php echo $nom_client; ?>"><img src="images/ImgDelete.gif" width="16" height="16"></a></div></td>
      <td><a href="edit_client.php?id=<?php echo $id_client; ?>" class="nyroModal"><?php echo $id_client.' - '.$prenom_client; ?> 
        <?php echo $nom_client; ?></a></td>
      <td><?php echo $login_client; ?></td>
      <td><?php echo $email_client; ?></td>
      <td colspan="3"><?php echo $tel_client; ?></td>
      <td align="center">
      	<?php
			if (@mysql_num_rows(mysql_query("SELECT * FROM factures WHERE id_client='$id_client'"))!=0) {
		?>
      	<a href="factures.php?id=<?php echo $id_client; ?>" title="Factures de <?php echo $prenom_client.' '.$nom_client; ?>" class="nyroModal">voir</a>
        <?php
			}
			else {
		?>
      ...
      <td align="center">
      <?php
			if (@mysql_num_rows(mysql_query("SELECT * FROM abonnements WHERE id_client='$id_client'"))!=0) {
		?>
      <a href="abonnes.php?id=<?php echo $id_client; ?>" title="Abonnements de <?php echo $prenom_client.' '.$nom_client; ?>" class="nyroModal">voir</a></td>
      <?php
			}
			else {
	  ?>
    ...
    <tr> 
      <td align="center">
        <?php
					if ($offset>0) {
				?>
        <a href="clients.php?offset=<?php echo $previousoffset ; ?>"> 
        <?php
                    	echo "Précédent";
					}
      ...
      <td align="center">
        <?php 
					...
					{	
						...
						{
				?>
        <a href="clients.php?offset=<?php echo $x ; ?>"> 
        <?php 
							echo ($page . " ");
				?>
        </a> 
      ...
      <td align="center">
        <?php 
					...
					{
				?>
        <a href="clients.php?offset=<?php echo $nextoffset; ?>"> 
        <?php
						echo "Suivant";
					}
