...
<script language="JavaScript" type="text/javascript">
	function supprimer(i,j) {
		if (confirm('Etes-vous sûr de vouloir supprimer '+j+' ?')) {
			document.location.href='cod_equipe.php?id='+i+'&opt=suppr';
		}
	}
	</script>
	<!--nyroModal-->
...
<body onselectstart="return false" oncontextmenu="return false" ondragstart="return false" onMouseOver="window.status=''; return true;">
  <table width="991px" border="0" align="center" cellpadding="2" cellspacing="1">
    ...
    <tr bgcolor="#000000" class="table_entete"> 
      ...
      <td>Equipes</td>
      <td>Compos&eacute;es de</td>
      <td><div align="center"><a href="edit_equipe.php?opt=new" class="nyroModal" title="Nouvelle &eacute;quipe"><img src="images/edit_add-32.png" width="16" height="16"></a></div></td>
    </tr>
    <?php
  	$lignes=20;
  ...
  ?>
    <tr <?php if ($i%2==0) echo 'bgcolor="#B0DBFD"'; else echo 'bgcolor="#D8EDFE"';?> class="table_contenu"> 
      ...
      <td><div align="center"> 
          <?php
		if (mysql_num_rows(mysql_query("SELECT * FROM personnel WHERE id_equipe='$id_equipe'"))==0) {
	?>
          <a href="javascript:;" onClick="supprimer('<?php echo $id_equipe; ?>','<?php echo $lib_equipe; ?>');" title="supprimer <?php echo $lib_equipe; ?>"><img src="images/ImgDelete.gif" width="16" height="16"></a> 
          <?php
		}
	?>
        </div></td>
      <td><a href="edit_equipe.php?id=<?php echo $id_equipe; ?>&opt=maj" title="<?php echo $lib_equipe; ?>" class="nyroModal"><?php echo $lib_equipe; ?></a></td>
      <td colspan="2"> 
        <?php
		$sql="SELECT * FROM personnel WHERE id_equipe='$id_equipe'";
		$result=mysql_query($sql);
		while ($row=mysql_fetch_array($result)) {
			extract($row);
			echo ' * <a href="edit_personnel.php?id='.$id_pers.'&opt=maj" title="'.$prenom_pers.' '.$nom_pers.'" class="nyroModal">'.$prenom_pers.' '.$nom_pers.'</a><br>';
		}
	?>
      </td>
    </tr>
    ...
    <tr> 
      <td align="center">
        <?php
					if ($offset>0) {
				?>
        <a href="equipes.php?offset=<?php echo $previousoffset ; ?>"> 
        <?php
                    	echo "Précédent";
					}
      ...
      <td align="center">
        <?php 
					...
					{	
						...
						{
				?>
        <a href="equipes.php?offset=<?php echo $x ; ?>"> 
        <?php 
							echo ($page . " ");
				?>
        </a> 
      ...
      <td align="center">
        <?php 
					...
					{
				?>
        <a href="equipes.php?offset=<?php echo $nextoffset; ?>"> 
        <?php
						echo "Suivant";
					}
