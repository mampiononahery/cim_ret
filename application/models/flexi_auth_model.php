...
class Flexi_auth_model extends Flexi_auth_lite_model
{
	...
	public function insert_user($email, $username, $password, $custom_data = FALSE, $group_id = FALSE)
	{
	    ...
	    $sql_insert = array(
			...
			// mampy///
			//$this->auth->tbl_col_user_account['prenom_user'] =>$username,
			...
			$this->auth->tbl_col_user_account['active'] => 1,	//mampy	
	 ...
	 * @author Rob Hussey
	 */
	public function get_user_privileges($sql_select, $sql_where,$id_user_remplacer= 0 )
	{
		// Set any custom defined SQL statements.
		$this->flexi_auth_lite_model->set_custom_sql_to_db($sql_select, $sql_where,$id_user_remplacer);
	...
	{	
		...
		$sql_where = array($this->auth->primary_identity_col => $identity,'actif'=>1);
	...
	private function set_login_sessions($user, $logged_in_via_password = FALSE)
	{
		...
		//var_dump($privilege_sources); exit();
        ...
        if (in_array('user', $privilege_sources))
        {
            ...
            $query = $this->get_user_privileges($sql_select, $sql_where,$id_user_remplacer);
